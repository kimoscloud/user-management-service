...
import (
	...
	organizationUserRepository "github.com/kimoscloud/user-management-service/internal/core/ports/repository/organization/user-organization"
...
type CreateOrganizationMemberUseCase struct {
	...
	organizationUserRepository organizationUserRepository.Repository
	logger                     logging.Logger
}

func (cu CreateOrganizationMemberUseCase) Handler(authenticatedUserId, orgId string, request *request.CreateOrganizationUsers) *errors.AppError {
	authenticatedOrgUser, err := cu.userOrganizationRepo.GetUserOrganizationByUserAndOrganizationWithRolesAndPermissions(authenticatedUserId, orgId)
	if err != nil {
		//TODO replace the error code here
		return errors.NewInternalServerError("Error inviting user to the organization",
...
func (cu CreateOrganizationMemberUseCase) checkIfAuthenticatedOrgUserHasPermission(user *organization.UserOrganization) {
